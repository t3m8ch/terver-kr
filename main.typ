...
В приложении 9 нет нужного значения $D_#[кр]$ для $n = 200$, поэтому вычислим его самостоятельно:

$
  D_#[кр] = D(0.05; 200) = 1.358 / (sqrt(n) + 0.12 + (0.11 / sqrt(n))) = \
  = 1.358 / (14.14 + 0.12 + (0.11 / 14.14)) approx 0.095
$

$
  D_#[кр] = D(0.05; 50) = 0.188\
  D_#[набл] = max |accent(F_0, ~) (x) - accent(F, ~)_n (x)| = 0.008
$

так как $D_#[набл] < D_#[кр]$, то гипотезу принимаем.

=== Критерий Пирсона

$
  chi^2_#[набл] = 0 + 0.028 + 0.012 + 0.062 + 0.136 = 0.238\
  chi^2_#[кр] = (0.05; 5 - 3) = 6
$

так как $chi^2_#[набл] < chi^2_#[кр]$, то гипотезу принимаем.
...
for i in range(len(x_i)):
    if i < len(x_i) - 1:
        ...
        plt.hlines(y=f_n[i], xmin=x_i[i], xmax=60, colors="black",
                   linewidth=2)
